...
function FileList() {
	this.files = [];
	this.info = {upload:[], fileId:[], removeId:[]}; //업로드 여부, 업로드된 파일id, 삭제할 파일id
	//이미 업로드된 파일 : 처리할 게 없음
	//새로 추가된 파일 : upload 해야 함
	//이미 업로드 되어 있던 파일 삭제 : DB + 물리적 파일 삭제
	//{}=new Object();
	//[]=new Array();
...
//		}
	...
	this.removeFile = function(i) {
		...
		//이미 업로드 되어 있는 파일 삭제 한 경우 삭제할 파일 id에 담기
		if(typeof this.info.fileId[i] != "undefined"){
			this.info.removeId.push(this.info.fileId[i]); //삭제 대상 파일id 추가
...
//파일첨부 정보 file태그에 담기
var singleFile = ''; //파일 선택시 선택한 첨부파일정보를 담아둘 변수
...
function modalAlert(type, title, message) {
	...
	if (type == 'danger') {//confirm에 해당
		...
		//확인버튼, confirm일 때만 사용할 btn-danger 버튼은 필요 없음.
...
//동적으로 만들어진 요소(태그)에 대해서는 document에 이벤트를 등록해야 한다.
$(document)
	.on('click', '.date + .date-delete', function() {
		$(this).css('display', 'none'); //삭제 버튼 안보이게 처리
...
function isImage(filename) {
	//png, jpg, txt, hwp
	var ext = filename.substr(filename.lastIndexOf('.') + 1).toLowerCase(); //확장자 소문자로 변경
	var imgs = ["png", "jpg", "bmp", "gif", "jpeg", "webp"];
	return imgs.indexOf(ext) == -1 ? false : true; //index값은 0부터 : 존재하지 않는 인덱스는 -1
}
...
//		var files = this.files;
		var files = filteredFile(this.files) //10M 초과 파일은 걸러내기
	...
	$('input#file-single').change(function() {
		...
		var _delete = $('#file-attach .file-delete'); //파일 삭제 요소
		...
		if (attached) {
			...
			if (isImage(attached.name)) {
				...
				if ($(this).hasClass("image-only")) {
					singleFile = '';//이미지가 아닌 파일인 경우는 관리 정보를 초기화
